...
export function ProductHistory({ id, api, version }) {
  ...
  return <details className="product-history"><summary>Change history</summary>
    ...
    <ul>{events.map((event, i) => <li key={i}><strong>{{save_product:'Product details saved',publish_product:'Published',unpublish_product:'Unpublished'}[event.action]}</strong> · {new Date(event.created_at).toLocaleString()}<br/><span>{event.actor}</span></li>)}</ul>
  </details>;
}

export function ProductTable({ items, collections, choose, api, refresh }) {
